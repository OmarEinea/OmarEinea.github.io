...
const get = (link, type) => fetch(link).then(type === 'text' ? text : json);
...
}).database().ref('profiles/Development');
...
const cache = img => storage.file(img).setMetadata(cacheHeader)
  .then(() => console.log('Updated', img)).catch(() => console.log('NoImage', img));
const forEach = (data, apply) => {
...
const cacheHeader = { cacheControl: 'public, max-age=' + 24 * 60 * 60 };
// const rect = /<rect .*?fill="#([0-9a-f]{6})".*?\/>/g, colors = [
